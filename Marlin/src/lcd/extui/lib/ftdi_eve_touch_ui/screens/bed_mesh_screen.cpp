...
void BedMeshScreen::onEntry() {
  ...
  screen_data.BedMesh.zAdjustment = 0;
...
float BedMeshScreen::getHighlightedValue(bool nanAsZero) {
  xy_uint8_t pt;
  if (tagToPoint(screen_data.BedMesh.highlightedTag, pt)) {
...
void BedMeshScreen::moveToHighlightedValue() {
  xy_uint8_t pt;
  if (tagToPoint(screen_data.BedMesh.highlightedTag, pt))
    ExtUI::moveToMeshPoint(pt, gaugeThickness + screen_data.BedMesh.zAdjustment);
}

void BedMeshScreen::adjustHighlightedValue(float increment) {
  screen_data.BedMesh.zAdjustment += increment;
  moveToHighlightedValue();
}

void BedMeshScreen::saveAdjustedHighlightedValue() {
  if(screen_data.BedMesh.zAdjustment) {
    BedMeshScreen::setHighlightedValue(BedMeshScreen::getHighlightedValue(true) + screen_data.BedMesh.zAdjustment);
    screen_data.BedMesh.zAdjustment = 0;
  }
}
...
void BedMeshScreen::drawHighlightedPointValue() {
  ...
  cmd.font(Theme::font_medium)
     ...
     .font(font_small);
  draw_adjuster(cmd, Z_VALUE_POS, 2, getHighlightedValue(true) + screen_data.BedMesh.zAdjustment, GET_TEXT_F(MSG_UNITS_MM), 4, 3);
